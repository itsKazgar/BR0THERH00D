...
def check():
    ...
    # 1. Drawdown check — use actual saved start balance, not hardcoded $100
    start_balance = state.get("day_start_balance", state.get("balance", START_BALANCE))
    drawdown = (start_balance - balance) / max(start_balance, 1)
    if drawdown > MAX_DRAWDOWN:
        alerts.append(f"⚠️  DRAWDOWN ALERT: portfolio down {drawdown:.0%} (${balance:.2f} vs start ${start_balance:.2f})")
    ...
    for mint, pos in positions.items():
        ...
        if price > 0 and entry > 0:
            ...
            if pnl < -20:
                alerts.append(f"🔴 {pos.get('name','?')} DOWN {pnl:.1f}% — consider manual exit")
